...
function keyboard._internal.keys:getKeyINT(_kstr)
	return self['KEY_' .. string.upper(_kstr)]
end

function keyboard.isDown(_key) 
	if type(_key) == 'number' then
		return raylib.IsKeyDown(_key)
	elseif type(_key) == 'string' then
		return raylib.IsKeyDown(keyboard._internal.keys:getKeyINT(_key))
	end
end
